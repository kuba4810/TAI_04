...
const userSchema = new mongoose.Schema({
  ...
  async function createNewOrUpdate(user) {
    ...
    return Promise.resolve().then(() => {
      if (!user.id) {
        console.log('User nie posiada id')
         new UserModel(user).save().then(result => {
          console.log("Dane zwrócone z bazy: ",result);
          if (result) {            
            console.log('Result hest ok');
            let newResult = mongoConverter(result);
            console.log('Przetworzony rezultat: ',newResult);
            
            return newResult;
          }
        });
      } else {
        console.log('User posiada id');
        return UserModel.findByIdAndUpdate(user.id, _.omit(user, 'id'), { new: true });
      }
    }).catch(error => {
      console.log('Złapany błąd !');
